...
(() => {
    ...
    let currentLimit = 5;
    let currentSearch = "";
    const listContainer = document.getElementById("testimonialListContainer");
    ...
    const fetchContent = async (url, container) => {
        try {
            ...
            if (container.id === "testimonialListContainer") attachListListeners(container);
            if (container.id === "testimonialFormContainer") {
    ...
    const loadPage = (page = 1) => {
        currentPage = page;
        getCurrentListState();
        const url = `/frontend/testimonial/testimonialList?page=${currentPage}&limit=${currentLimit}&search=${currentSearch}`;
    ...
    const deleteTestimonial = async (id) => {
        showCustomConfirm(
            ...
            async () => {
                const res = await fetch(`/frontend/testimonial/delete/${id}`, { method: "DELETE" });
    ...
    const loadEditForm = (id) => {
        switchTab(document.getElementById("testimonialCreateTab"));
        fetchContent(`/frontend/testimonial/edit/${id}`, formContainer);
    };
    ...
    const handleFormSubmission = () => {
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    showToast("success", data.message);
                    switchTab(document.getElementById("testimonialListTab"));
                } else {
    ...
    const setupPhotoDragDrop = () => {
        const dropArea = document.getElementById("testimonialPhotoDropArea");
        const fileInput = document.getElementById("testimonialPhotoInput");
        const previewContainer = document.getElementById("testimonialPhotoPreview");
    ...
    const attachListListeners = (container) => {
        ...
        container.querySelectorAll(".btn-delete").forEach((btn) =>
            btn.addEventListener("click", (e) => {
                e.preventDefault();
                deleteTestimonial(e.currentTarget.dataset.id);
            })
        );
    ...
    const listTab = document.getElementById("testimonialListTab");
    const createTab = document.getElementById("testimonialCreateTab");
    ...
    const switchTab = (activeTab) => {
        ...
        if (activeTab === listTab) {
            ...
            fetchContent("/frontend/testimonial/testimonialUI", formContainer);
        }
    };
    ...
    document.addEventListener("DOMContentLoaded", () => {
        if (!listTab || !createTab || !listContainer || !formContainer) {
            console.error("Testimonial containers or tabs not found.");
            return;
        }
