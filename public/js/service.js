...
(() => {
    ...
    const fetchContent = async (url, container) => {
        try {
            ...
            if (container.id === "serviceListContainer") attachListListeners(container);
            if (container.id === "serviceFormContainer") {
    ...
    const loadPage = (page = 1) => {
        currentPage = page;
        getCurrentListState();
        const url = `/frontend/services/serviceList?page=${currentPage}&limit=${currentLimit}&search=${currentSearch}`;
    ...
    const deleteService = async (id) => {
        showCustomConfirm(
            ...
            async () => {
                const res = await fetch(`/frontend/services/delete/${id}`, { method: "DELETE" });
    ...
    const loadEditForm = (id) => {
        switchTab(document.getElementById("serviceCreateTab"));
        fetchContent(`/frontend/services/edit/${id}`, formContainer);
    };
    ...
    const handleFormSubmission = () => {
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    showToast("success", data.message);
                    switchTab(document.getElementById("serviceListTab"));
                } else {
    ...
    const listTab = document.getElementById("serviceListTab");
    const createTab = document.getElementById("serviceCreateTab");
    ...
    const switchTab = (activeTab) => {
        ...
        if (activeTab === listTab) {
            ...
            fetchContent("/frontend/services/serviceUI", formContainer);
        }
    };
    ...
    document.addEventListener("DOMContentLoaded", () => {
        if (!listTab || !createTab || !listContainer || !formContainer) {
            console.error("Services containers or tabs not found.");
            return;
        }
